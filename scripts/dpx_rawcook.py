...
log(logfile, "DPX folder will be cooked using --output-version 2:") #Change this dumb logic
if cook_retry:
    with open(temp_retry_file, 'w') as file:
        file.writelines(item + "\n" for item in cook_retry if cook_retry)

    log(logfile, (item + "\n" for item in cook_retry if cook_retry))

# Begin RAWcooked processing with GNU Parallel using --output-version 2
command = "rawcooked --license 00C5BAEDE01E98D64496F0 -y --all --no-accept-gaps --output-version 2 -s 5281680 {DPX_PATH}{argument} -o {MKV_DEST}mkv_cooked/{argument}.mkv &>> {MKV_DEST}mkv_cooked/{argument}.mkv.txt"
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:          #Change to parallel processes rather than threads
        futures = [executor.submit(command, argument) for argument in filenames]
...
log(logfile, "DPX folder will be cooked:")  #Change this dumb logic
...
command = "rawcooked --license 00C5BAEDE01E98D64496F0 -y --all --no-accept-gaps -s 5281680 {DPX_PATH}{argument} -o {MKV_DEST}mkv_cooked/{argument}.mkv &>> {MKV_DEST}mkv_cooked/{argument}.mkv.txt"
...
command = "rawcooked --license 00C5BAEDE01E98D64496F0 -y --all --no-accept-gaps -s 5281680 --framemd5 {DPX_PATH}{argument} -o {MKV_DEST}mkv_cooked/{argument}.mkv &>> {MKV_DEST}mkv_cooked/{argument}.mkv.txt"
with open (rawcook_file,'r') as file:
    filenames = file.read().splitlines()
