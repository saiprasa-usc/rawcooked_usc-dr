    ...
    if not queued_pass and not queued_fail:
        ...
        if check_str.startswith('pass!'):
            ...
            if pixels_per_line > 3999:
                log(logfile, "PASS: 4K scan" + file_scan_name + "has passed the MediaConch policy and can progress to RAWcooked processing path")
                ...
                if luma_match > 0:
                    log(logfile, "PASS: Luma (Y) $file_scan_name has passed the MediaConch policy and can progress to RAWcooked processing path")
                    with open(luma_4k_dpx_file, 'w') as file:
                        file.write(DPX_PATH+filename)
                else:
                    log(logfile, "PASS: RGB $file_scan_name has passed the MediaConch policy and can progress to RAWcooked processing path")
                    with open(rawcooked_dpx_file, 'w') as file:
                        file.write(DPX_PATH+filename)
                
        else:
            log(logfile, "FAIL: " + file_scan_name + " DOES NOT CONFORM TO MEDIACONCH POLICY. Adding to tar_dpx_failures_list.txt")
